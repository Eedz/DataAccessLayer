...
namespace ITCLib
{
    partial class DBAction
    {
        ...
        /// <param name="SurveyCode"></param>
        /// <returns></returns>
        public static List<string> GetSavedSources(int userid)
        {
            List<string> sourceList = new List<string>();
            string query = "SELECT * FROM qrySavedSources WHERE PersonnelID = @userid ORDER BY SourceNumber ASC";
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ISISConnectionString"].ConnectionString))
            {
                ...
                try
                {
                    using (SqlDataReader rdr = sql.SelectCommand.ExecuteReader())
                    {
                        while (rdr.Read())
                        {
                            sourceList.Add((string)rdr["SourceText"]);
                        }
                    }
                }
                catch (Exception e)
        ...
        public static List<Comment> GetSavedComments(int userid)
        {
            List<Comment> commentList = new List<Comment>();
            string query = "SELECT * FROM qrySavedComments WHERE PersonnelID = @userid ORDER BY ID ASC";
                ...
                try
                {
                    using (SqlDataReader rdr = sql.SelectCommand.ExecuteReader())
                    {
                        while (rdr.Read())
                        {
                            ...
                            c.ID = (int)rdr["ID"];
                            c.Notes = new Note(0, rdr.SafeGetString("Comment"));

                            c.NoteDate = rdr.SafeGetDate("NoteDate");
                            if (c.NoteDate == null)
                            {
                                c.NoteDate = DateTime.Today;
                            }

                            c.Author.ID = (int)rdr["NoteInit"];
                            c.Author.Name = rdr.SafeGetString("Name");

                            c.SourceName = rdr.SafeGetString("SourceInit");
                            c.Authority.ID = (int)rdr["AuthorityID"];
                            c.Authority.Name = rdr.SafeGetString("Authority");
                            
                            c.NoteType.ID = (int)rdr["NoteTypeID"];
                            c.NoteType.TypeName = rdr.SafeGetString("CommentType");

                            c.Source = rdr.SafeGetString("Source");
        ...
        public static Comment GetLastUsedComments(int userid)
        {
            Comment c = new Comment();
            string query = "SELECT * FROM qryLastUsedComment WHERE PersonnelID = @userid";
                ...
                try
                {
                    using (SqlDataReader rdr = sql.SelectCommand.ExecuteReader())
                    {
                        while (rdr.Read())
                        {
                            c.ID = (int)rdr["ID"];
                            c.NoteDate = rdr.SafeGetDate("NoteDate");
                            c.SourceName = rdr.SafeGetString("SourceName");
                            c.Source = rdr.SafeGetString("Source");
                            c.Author.ID = (int)rdr["NoteInit"];
                            c.Author.Name = rdr.SafeGetString("Name");
                            c.NoteType.ID = (int)rdr["NoteTypeID"];
